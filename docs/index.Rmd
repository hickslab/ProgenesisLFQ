...
data <- pepm %>%
  ...
  filter_redox_tidy(reduced = "Nethylmaleimide") %>%
...
data4 <- data4 %>%
  ...
  left_join(data4, ., by = data4 %>% select(1) %>% names())
...
```{r}
data2 %>% plot_pca(., group)# + theme_custom()
...
data4 %>%
  ...
  plot_hclust(., group, k = 6, type = "line") +
  theme_custom()
...
> Assuming mapped UniProtKB annotations.
